<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <x-input-label for="blood_group" :value="__('blood_group')" />
            <x-text-input id="	blood_group" name="	blood_group" type="text" class="mt-1 block w-full" :value="old('	blood_group', $user->blood_group)" required autofocus autocomplete="	blood_group" />
            <x-input-error class="mt-2" :messages="$errors->get('	blood_group')" />
        </div>

        <div>
            <x-input-label for="department_name" :value="__('department_name')" />
